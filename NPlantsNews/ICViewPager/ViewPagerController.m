...
#define kTabsViewBackgroundColor [UIColor whiteColor]
...
//        [[[MenuManager sharedManager].colorArray objectAtIndex:[MenuManager sharedManager].currentIndex] setStroke];
//        [bezierPath stroke];
    //}
}
@end
...
@property (nonatomic) BOOL lockFlg;
...
//        }
    ...
    frame.size.height = [self.tabHeight floatValue] + 1;
    ...
    frame.size.width = CGRectGetWidth(self.view.frame);
    frame.size.height = CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) + (self.tabBarController.tabBar.hidden ? CGRectGetHeight(self.tabBarController.tabBar.frame) : 0);
    self.contentView.frame = frame;
}

#pragma mark - IBAction
- (IBAction)handleTapGesture:(id)sender {
    [MenuManager sharedManager].touchMenuFlg = YES;
    ...
    NewsContentViewController *controller = (NewsContentViewController *)viewController;
    int temp = [[[NSUserDefaults standardUserDefaults] objectForKey:@"index"] intValue];
    [MenuManager sharedManager].lastIndex = temp;
    [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%d", (int)index] forKey:@"index"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [MenuManager sharedManager].currentController = controller;
    
    // frame保存
    CGRect lastFrame = [MenuManager sharedManager].nextTabframe;
    [MenuManager sharedManager].lastTabframe = lastFrame;
    CGRect nextFrame = tabView.frame;
    [MenuManager sharedManager].nextTabframe = nextFrame;
    ...
    if (self.activeTabIndex != index) {
        ...
        if (self.activeTabIndex == [MenuManager sharedManager].bodyMenuIndex || self.activeTabIndex == [MenuManager sharedManager].loveMenuIndex) {
            [self selectTabAtIndex:index didSwipe:NO];
        }
    }
    
    if (index == [MenuManager sharedManager].bodyMenuIndex && [MenuManager sharedManager].lastIndex == [[[NSUserDefaults standardUserDefaults] objectForKey:@"index"] intValue]) {
        [self.delegate showBodySubmenu];
    } else if (index == [MenuManager sharedManager].loveMenuIndex && [MenuManager sharedManager].lastIndex == [[[NSUserDefaults standardUserDefaults] objectForKey:@"index"] intValue]) {
        [self.delegate showLoveSubmenu];
    }
}
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    //[MenuManager sharedManager].currentTabframe = frame;
...
- (NSNumber *)tabOffset {
    ...
    if (!_tabOffset) {
        CGFloat value = [MenuManager sharedManager].tabMoveSet;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
        ...
        int value = screenWidth/2-[MenuManager sharedManager].nextTabframe.size.width/2;
...
//    
//    if (!_tabWidth) {
//        NSNumber *number = [[MenuManager sharedManager].tabWidthArray objectAtIndex:self.activeTabIndex];
...
- (void)setNeedsReloadOptions {
    ...
    self.tabWidth = [NSNumber numberWithFloat:[self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:80.0]];
    self.tabOffset = [NSNumber numberWithFloat:[self.delegate viewPager:self valueForOption:ViewPagerOptionTabOffset withDefault:[MenuManager sharedManager].tabMoveSet]];
...
- (void)defaultSetup {
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        CGRect frame = tabView.frame;
        frame.origin.x = contentSizeWidth;
        NSNumber *number = [[MenuManager sharedManager].tabWidthArray objectAtIndex:i];
    ...
    NSUInteger index = [[[NSUserDefaults standardUserDefaults] objectForKey:@"index"] intValue];
    [self selectTabAtIndex:index didSwipe:NO];
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        NSNumber *number = [[MenuManager sharedManager].tabWidthArray objectAtIndex:index];

        // Create TabView and subview the content
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, [number floatValue], [self.tabHeight floatValue])];
        
        // 灰色線
        if (index < 9) {
            UIView *view = [[UIView alloc] initWithFrame:CGRectMake([number intValue]-1,18.5,1,7.0)];
